...
public class EditAccountCompleteAction extends LookupDispatchAction {
    ...
    /**
     * 管理画面 アカウント編集 確認画面で"修正"ボタンを押下した場合に入力画面にフォワードするためのメソッド
    ...
    public ActionForward fix(
        ...
        try {
            ...
            // パスワード変更フラグにfalseを設定する。
            inForm.setChangePasswordFlag(false);
    ...
    /**
     * 管理画面 アカウント編集 確認画面で"編集"ボタンを押下した場合に完了画面にフォワードするためのメソッド
     * @param mapping アクションマッピング
     * @param form アクションフォーム
     * @param request リクエスト
     * @param response レスポンス
    ...
    public ActionForward edit(
        ...
        try {
            ...
            accountUpdateDataForm.setAccountStatus(inForm.getAccountStatus()); // アカウントステータス
            ...
            if(inForm.isChangePasswordFlag()) {
                // パスワード変更フラグがtrueの場合は新しいパスワードを安全なパスワードに変換してアクションフォームに格納する。
                accountUpdateDataForm.setPassword(
                         new PasswordUtil().getSafetyPassword(inForm.getNewPassword(),
                         inForm.getMailAddress()));
            }
            accountUpdateDataForm.setAccountStatus(inForm.getAccountStatus()); // アカウントステータス
