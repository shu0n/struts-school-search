...
public class RegisterSchoolCompleteAction extends LookupDispatchAction {
    ...
    /**
     * 管理画面 スクール登録 確認画面で"修正"ボタンを押下した場合に入力画面にフォワードするためのメソッド
    ...
    /**
     * 管理画面 スクール登録 確認画面で"登録"ボタンを押下した場合に完了画面にフォワードするためのメソッド
     * @param mapping アクションマッピング
     * @param form アクションフォーム
     * @param request リクエスト
     * @param response レスポンス
    ...
    public ActionForward register(
        ...
        try {
            if("account".equals(inForm.getRegistrantKind())) {
                // 登録者種別が"アカウント"の場合は
                // 登録者アカウントID(文字列型)を登録者アカウントID(整数型)に変換してアクションフォームに格納する。
            ...
            schoolInsertDataForm.setRegistrantAdminId(inForm.getRegistrantAdminId()); // 登録者管理者ID
            ...
            schoolInsertDataForm.setSchoolCity(inForm.getSchoolCity()); // スクール市区町村
