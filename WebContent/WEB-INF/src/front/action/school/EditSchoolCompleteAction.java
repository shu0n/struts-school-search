...
public class EditSchoolCompleteAction extends LookupDispatchAction {
    ...
    /**
     * スクール編集 確認画面で"修正"ボタンを押下した場合に入力画面にフォワードするためのメソッド
    ...
    public ActionForward edit(
        ...
        try {
            ...
            inForm = (FrontEditSchoolActionForm) new SchoolFileModel().setUpdateFileName(inForm);

            // DAOメソッドに引き渡すアクションフォームを生成して登録するデータを格納する。
            SchoolUpdateDataActionForm schoolUpdateDataForm = new SchoolUpdateDataActionForm();
            schoolUpdateDataForm.setSchoolId(inForm.getSchoolId()); // スクールID
