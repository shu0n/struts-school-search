...
public class DeleteReceivedMessageAction extends Action {
    ...
    public ActionForward execute(
        ...
        try {
            ...
            if(!new LoginStatusModel().isLogined(session)) {
                ...
                return new RedirectUtil().getRedirectLoginAction("/deleteReceivedMessage.do", "messageId", messageId);
            }
            ...
            // メッセージを削除する。
            new DeleteMessageDAO().deleteReceivedMessageLogically(messageId);

            // セッションからアクションフォームを削除する。
            session.removeAttribute("FrontDeleteReceivedMessageActionForm");
            // 受信メッセージ一覧画面にフォワードする。
