...
public class SelectSchoolJoinDAO {
    Connection connection = null; // DB接続用の変数

    /**
     * スクールテーブルをカテゴリーテーブル、都道府県テーブル、アカウントテーブルと外部結合して指定した検索条件に一致するレコードを取得するためのメソッド
     ...
     * @throws SQLException
     */
    public List<SchoolExtendActionForm> selectMatchedSchool(SchoolSelectJoinWhereActionForm whereForm)
            throws ClassNotFoundException, IOException, SQLException {
        try {
            // 外部結合するためのSELECT文を生成する。
            String joinSql
                    = "SELECT * FROM ((("
                    + "schools LEFT OUTER JOIN categories ON schools.school_category_id=categories.category_id) "
            ...
            if(!StringUtils.isEmpty(schoolReleasePropriety)) {
                ...
                if(schoolReleaseProprietyArray.length == 1) {
                ...
                } else if(schoolReleaseProprietyArray.length >= 2) {
                    // 上記以外の場合
                    ...
                    for(int i = 0; i < schoolReleaseProprietyArray.length; i++) {
                        ...
                        schoolReleaseProprietySb.append("OR school_release_propriety='" + schoolReleaseProprietyArray[i] + "' ");
                    }
                    // 先頭3文字を削除する
            ...
            if(!StringUtils.isEmpty(schoolEntryPropriety)) {
                ...
                } else if(schoolEntryProprietyArray.length >= 2) {
                    // 上記以外の場合
            ...
            String sql = joinSql + whereSb.toString() + "ORDER BY school_id ASC";
            ...
            while(rs.next()) {
                ...
                SchoolExtendActionForm outForm = new SchoolExtendActionForm();
                outForm.setSchoolId(rs.getInt("school_id")); // スクールID
                outForm.setRegistrantAccountId(rs.getInt("registrant_account_id")); // 登録者アカウントID
                outForm.setRegistrantLastName(rs.getString("last_name")); // 登録者姓
                outForm.setRegistrantFirstName(rs.getString("first_name")); // 登録者名
                outForm.setRegistrantLastNameKana(rs.getString("last_name_kana")); // 登録者姓(フリガナ)
                outForm.setRegistrantFirstNameKana(rs.getString("first_name_kana")); // 登録者名(フリガナ)
                ...
                outForm.setSchoolFee(new BigDecimal(rs.getString("school_fee"))); // スクール費用
                ...
                // リストにアクションフォームを追加する。
                schoolExtendFormList.add(outForm);
            }
