...
public class SelectContactDAO {
    Connection connection = null; // DB接続用の変数

    /**
     * お問合せテーブルから指定した検索条件に一致するレコードを取得するためのメソッド
     ...
     * @throws SQLException
     */
    public List<ContactActionForm> selectMatchedContact(ContactSelectWhereActionForm whereForm)
            throws ClassNotFoundException, IOException, SQLException {
        try {
            // テーブルを指定するSELECT文を生成する。
            String tableSql = "SELECT * FROM contacts ";
            ...
            if(contactStatusId != 0) {
                ...
                if(contactStatusIdArray.length == 1) {
                    ...
                    for(int i = 0; i < contactStatusIdArray.length; i++) {
                        ...
                        contactStatusIdSb.append("OR contact_status_id=" + contactStatusIdArray[i] + " ");
                    }
                    // 先頭3文字を削除する
            ...
            String sql = tableSql + whereSb.toString() + "ORDER BY contact_id ASC";
            ...
            List<ContactActionForm> contactFormList = new ArrayList<>();
            // 取得したレコードの件数分、処理を繰り返す。
            while(rs.next()) {
                // レコードから各列のデータを取得してアクションフォームに格納する。
                ContactActionForm outForm = new ContactActionForm();
