<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="admin.actionform.account.AdminEditAccountActionForm"%>
...
AdminEditAccountActionForm inForm = (AdminEditAccountActionForm) session.getAttribute("AdminEditAccountActionForm");
%>
<!DOCTYPE html>
<html lang="ja">
...
<section class="container-fluid pt-4" style="width: 90%;">
    <h2>アカウント編集<br class="d-sm-none" />(入力)</h2>
</section>
<section class="container-fluid pt-4" style="width: 90%;">
    <div>
        <%
        // スクール登録 入力画面へのURLを生成する。
        String registerSchoolUrl = "return !isDoubleClick('./registerSchoolInput.do?accountId=" + inForm.getAccountId() +"')";
        %>
        <html:button property="registerSchoolBtn" value="スクール登録ページへ" styleClass="btn btn-primary" onclick="<%=registerSchoolUrl %>"/>
    </div>
</section>
<section class="container-fluid pt-4" style="width: 90%;">
    <html:form action="/editAccountConfirm" enctype="multipart/form-data" onsubmit="return validateBeforeSubmitAccount()">
        ...
        <fieldset class="form-group">
            <div class="row">
                <legend class="col-md-3 col-form-label">アカウントステータス</legend>
                <div class="col-md-9" style="padding-top: calc(0.375rem + 1px);">
                    <%
                    int i = 1;
                    %>
                    <logic:iterate id="accountStatusMap" name="AdminEditAccountActionForm" property="accountStatusMap">
                        <div class="form-check form-check-inline">
                            <%
                            // for属性の値を生成する。
                            String accountStatusIndex = "accountStatus" + i;
                            i += 1;
                            %>
                            <html:radio idName="accountStatusMap" property="accountStatus" value="key" styleClass="form-check-input" styleId="<%=accountStatusIndex %>" onblur="validateRadioId('accountStatus')"/>
                            <label class="form-check-label" for="<%=accountStatusIndex %>"><bean:write name="accountStatusMap" property="value"/></label>
                        </div>
                    </logic:iterate>
                    <div class="invalid-feedback">アカウントステータスを選択してください。</div>
                </div>
            </div>
        </fieldset>
        <div class="form-group row">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-4 input-group">
                ...
                <div class="invalid-feedback">40文字以内のカタカナで入力してください。</div>
            </div>
            <div class="col-md-4 input-group">
                <div class="input-group-prepend">
        ...
        <fieldset class="form-group">
            <div class="row">
                ...
                <div class="col-md-9" style="padding-top: calc(0.375rem + 1px);">
                    ...
                    int j = 1;
                    %>
                    <logic:iterate id="sexMap" name="AdminEditAccountActionForm" property="sexMap">
                        <div class="form-check form-check-inline">
                            <%
                            // for属性の値を生成する。
                            String sexIdIndex = "sexId" + j;
                            j += 1;
                            %>
        ...
        if(!StringUtils.isEmpty(profileImageFilePath)) {
            // 存在する場合はimgタグのsrc属性に設定する。
            profileImageFilePath = request.getContextPath() + profileImageFilePath;
        %>
            <div class="form-group row">
                <label class="col-md-3 col-form-label"></label>
                <div class="col-md-4">
                    <html:checkbox property="deleteProfileImageFileFlag" value="true">プロフィール画像を削除する。</html:checkbox>
                    <div class="img-fluid">
                        <html:img src="<%=profileImageFilePath %>"/>
                    </div>
                </div>
            </div>
        <%
        ...
        <fieldset class="form-group">
            <div class="row">
                <legend class="col-md-3 col-form-label">パスワードを変更する</legend>
                <div class="col-md-9" style="padding-top: calc(0.375rem + 1px);">
                    <div class="form-check form-check-inline">
                        <html:checkbox property="changePasswordFlag" value="true" styleClass="form-check-input" styleId="isPasswordChanged" onchange="enableElement('isPasswordChanged', 'divPassword', 'password'); enableElement('isPasswordChanged', 'divConfirmPassword', 'confirmPassword')"/>
                    </div>
                </div>
            </div>
        </fieldset>
        <div class="form-group row" id="divPassword" style="display: none;">
            ...
            <div class="col-md-9">
                <html:password property="newPassword" redisplay="false" disabled="true" styleClass="form-control" styleId="password" onblur="validatePassword('password', 8, 16)"/>
                <div class="invalid-feedback">8〜16文字の半角英数字を入力してください。</div>
            </div>
        </div>
        <div class="form-group row" id="divConfirmPassword" style="display: none;">
            <label for="newConfirmPassword" class="col-md-3 col-form-label">新しいパスワード(確認)</label>
             <div class="col-md-9">
                <html:password property="newConfirmPassword" redisplay="false" disabled="true" styleClass="form-control" styleId="confirmPassword" onblur="validateConfirmPassword('password', 'confirmPassword')"/>
                <div class="invalid-feedback">パスワードが一致しません。</div>
            </div>
        </div>
        <div class="form-group row justify-content-end">
            <div class="col-md-9">
                <html:button property="listAccountBtn" value="キャンセル" styleClass="btn btn-secondary" onclick="return !isDoubleClick('./listAccount.do')"/>
                <html:submit value="確認する" styleClass="btn btn-primary"/>
                <%
                // アカウント削除 確認画面へのURLを生成する。
                String deleteAccountUrl = "return !isDoubleClick('./deleteAccountConfirm.do?accountId=" + inForm.getAccountId() + "')";
                %>
                <html:button property="deleteAccountBtn" value="削除する" styleClass="btn btn-danger" onclick="<%=deleteAccountUrl %>"/>
            </div>
        </div>
    </html:form>
</section>
